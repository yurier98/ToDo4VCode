...
### Fixed
- Critical packaging bug: `minimatch` (and its dependencies) were missing from the packaged VSIX, causing the extension to fail activation and not load tasks. The `.vscodeignore` now includes the runtime dependency chain.
...
### Fixed
- README preview screenshots switched from `webp` to `png` for better compatibility in VS Code extension details and marketplace rendering.
- Marketplace badge links in README aligned to stable providers and extension identifier casing.
